...
<!--wyk1120修改不弹出框-->
    ...
    <el-dialog :title="$t('adaptive.allset')" :visible.sync="dialogFormVisible" width="30%" :close-on-click-modal="false">
      <el-form :model="WANForm" label-position="left" size="small">
        <el-form-item :label="$t('adaptive.selectFunc')" :label-width="formLabelWidth" v-loading="WANLANLoading">
          <el-select v-model="WANForm.use" :placeholder="$t('placeholder.selectNet')">
            <el-option :disabled="isInuseLAN" :label="$t('adaptive.lan')" value="lan"></el-option>
            <el-option :disabled="isInuseWAN" :label="$t('adaptive.wan')" value="wan"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <p v-if="this.lanLimit <= this.lanCount && !WANLANLoading" style="color: red;">{{$t('adaptive.lanup')}}</p>
      <p v-if="this.wanLimit <= this.wanCount && !WANLANLoading" style="color: red;">{{$t('adaptive.wanup')}}</p>
      <div slot="footer" class="dialog-footer">
...
export default {
    ...
    checked: function() {
      if (this.checked !== 200) {
        if (this.$store.state.app.language === 'zh') {
          ...
          this.$confirm('Are you sure to adapt ports?', 'Notice', {
  ...
  methods: {
    ...
    WANLANSubmit: function() {
      ...
      getPorts().then(res => {
        for (let i = 0; i < res.data.interfaces.length; i++) {
          if (res.data.interfaces[i].webindex === bara.index) {
            para.webnetif = res.data.interfaces[i].webname
          }
        }
      })
      // WYK 改为发送webname
    ...
    unbind: function() {
      ...
      getPorts().then(res => {
        for (let i = 0; i < res.data.interfaces.length; i++) {
          if (res.data.interfaces[i].webindex === this.WANForm.index) {
            this.sendForm.webnetif = res.data.interfaces[i].webname
          }
        }
      })               // WYK转为发送webname
    ...
    sortingNextStep: function() {
      ...
      if (this.portsName.name.length > 10) {
        if (this.$store.state.app.language === 'zh') {
          ...
          this.$message({
            message: 'Portname should be controlled within 10 words',
            type: 'erro'
          })
        }
