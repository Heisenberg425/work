...
#else
      /* change group now, forever. uid we do later */
      if ( setgid (zprivs_state.zgid) )
        {
          fprintf (stderr, "zprivs_init: could not setgid, %s\n",
                    safe_strerror (errno) );
          exit (1);
        }
#endif
...
#else
      if ( seteuid (zprivs_state.zuid) )
        {
          fprintf (stderr, "privs_init (uid): could not seteuid, %s\n",
                   safe_strerror (errno));
          exit (1);
        }
#endif
  ...
  if (zprivs_state.zuid)
    {
      ...
      if ( seteuid (zprivs_state.zuid) )
        {
          fprintf (stderr, "privs_terminate: could not seteuid, %s",
                     safe_strerror (errno) );
          exit (1);
        }
#endif
