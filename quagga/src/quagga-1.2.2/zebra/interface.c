...
if_zebra_linkdetect_set (struct interface *ifp)
{
  ...
  if (!if_is_operative(ifp) && if_was_operative) if_down_zebra(ifp);
  /* Alternatively, it may come up after disabling link detection */
  if (if_is_operative(ifp) && !if_was_operative) if_up_zebra(ifp);
#else
  /* When linkdetection is enabled, interface might come down */
...
void
#ifdef SYLIXOS
if_up_zebra (struct interface *ifp)
#else
...
void
#ifdef SYLIXOS
if_down_zebra (struct interface *ifp)
#else
