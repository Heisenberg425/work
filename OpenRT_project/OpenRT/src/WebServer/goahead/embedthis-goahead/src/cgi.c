...
#if BIT_UNIX_LIKE && !defined(SYLIXOS) || QNX
/*
...
static int launchCgi(char *cgiPath, char **argp, char **envp, char *stdIn, char *stdOut)
{
    pid_t pid, fdin, fdout, hstdin, hstdout;
    ...
    if (posix_spawn_file_actions_addopen(&file_actions, STDIN_FILENO,
                                         stdIn, O_RDWR | O_CREAT, 0666)) {
    ...
    if (posix_spawn_file_actions_addopen(&file_actions, STDOUT_FILENO,
                                         stdOut, O_RDWR | O_CREAT, 0666)) {
        posix_spawn_file_actions_destroy(&file_actions);
        posix_spawnattr_destroy(&attr);
        return -1;
    }

    posix_spawn(&pid, cgiPath, &file_actions, &attr, argp, envp);
