...
int          listens[WEBS_MAX_LISTEN];   /* Listen endpoints */;
int          listenMax;
...
static void initWebs(Webs *wp, int flags, int reuse)
{
    ...
    strncpy(ipaddr, wp->ipaddr, sizeof(ipaddr));
    strncpy(ifaddr, wp->ifaddr, sizeof(ifaddr));
    memset(wp, 0, sizeof(Webs));
    strncpy(wp->ipaddr, ipaddr, strlen(ipaddr));
    strncpy(wp->ifaddr, ifaddr, strlen(ifaddr));
...
static void setFileLimits()
{
#if BIT_UNIX_LIKE && !defined(SYLIXOS)
    struct rlimit r;
    int           i, limit;
