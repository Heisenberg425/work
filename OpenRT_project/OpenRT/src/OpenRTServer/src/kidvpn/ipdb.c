...
ipdb_meta_data *parse_meta_data(const char *meta_json) {
...
#ifdef SYLIXOS
    cJSON    *obj = cJSON_Parse(meta_json);
    cJSON    *value;
    int      i;
    value = cJSON_GetObjectItem(obj, "node_count");
    meta_data->node_count = value->valueint;
    value = cJSON_GetObjectItem(obj, "total_size");
    meta_data->total_size = value->valueint;
    value = cJSON_GetObjectItem(obj, "build");
    meta_data->build_time = value->valuedouble;
    value = cJSON_GetObjectItem(obj, "ip_version");
    meta_data->ip_version = (short) (value->valueint);
    value = cJSON_GetObjectItem(obj, "fields");
    meta_data->fields_length = cJSON_GetArraySize(value);
    meta_data->fields = (char **) malloc(sizeof(char *) * meta_data->fields_length);
    for (i = 0; i < meta_data->fields_length; ++i) {
        cJSON *it = cJSON_GetArrayItem(value, i);
        meta_data->fields[i] = malloc(sizeof(char) * strlen(it->valuestring) + 1);
        strcpy(meta_data->fields[i], it->valuestring);
    }

    value = cJSON_GetObjectItem(obj, "languages");
    meta_data->language_length = cJSON_GetArraySize(value);
    ...
    for (i = 0; i < meta_data->language_length; ++i) {
        /*
         *  从文件中自己读出来的，cjson 没有对应的接口, 免费版本的只有中文
         */
        strcpy(meta_data->language[i].name, "CN");
        meta_data->language[i].offset = 0;
    }
